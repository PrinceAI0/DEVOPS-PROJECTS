...
- Let’s get the location of Node.js software from Ubuntu repositories.
  ...
  ![alt text](<Images/step 1.1.png>)

- Found out the setup_18.x was an older version of it as seen above in the image so i updated it to the lastes version by running the command below:

   ``` curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -```

   ![alt text](<Images/step1.1 updated.png>)
...
In Routes directory, open ```api.js``` with ```vim api.js```, delete the code inside with ```:%d``` command and paste the code below into it then save and exit with ```:wq```.
...
[Mongodb](https://www.mongodb.com/cloud/atlas/register).  Follow the sign up process, select **AWS** as the cloud provider, and choose a region near you.
...
- However, we do not have a frontend yet.
That means there is no website or user interface where a user can click buttons or type tasks.

- Because of this, we still need a way to check if our backend is working correctly during development.

- To do that, we use a tool called an ```API development client.```
This type of tool allows us to send requests (like GET, POST, PUT, DELETE) directly to our backend and see the responses, without needing a frontend.
